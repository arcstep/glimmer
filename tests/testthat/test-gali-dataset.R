test_that("<gali_read / gali_collect>", {
  sample_config_init()
  ...
  task_create("cars/gali_read") |>
    ...
    task_item_gali_add(gali_collect()) |>
    task_run() |>
    nrow() |>
    testthat::expect_equal(10)
...
test_that("<gali_filter>：一般流程", {
  ...
  ## 使用新的变量名
  gali_filter("cyl", ">", 6, s_dataName = "mydata") |>
...
test_that("<gali_filter>：正则表达式", {
  gali_filter("rowname", "%regex%", "Mazda") |>
  ...
  gali_filter("rowname", "%not-regex%", "Mazda") |>
    task_run_dp(`@result` = mtcars |> rownames_to_column()) |> nrow() |>
    testthat::expect_equal(
      mtcars |>
        rownames_to_column() |>
...
test_that("<gali_filter>：包含", {
  gali_filter("rowname", "%in%", c("Honda Civic", "Fiat 128")) |>
    task_run_dp(`@result` = mtcars |> rownames_to_column()) |> nrow() |>
    testthat::expect_equal(
      mtcars |>
        rownames_to_column() |>
  ...
  gali_filter("rowname", "%nin%", c("Honda Civic", "Fiat 128")) |>
    task_run_dp(`@result` = mtcars |> rownames_to_column()) |> nrow() |>
    testthat::expect_equal(
      mtcars |>
        rownames_to_column() |>
...
test_that("<gali_filter>：比较时间和日期", {
  ...
  ## 测试日期格式比较
  gali_filter("day", "date# >", "2020-05-30") |> task_run_dp(`@result` = d) |> nrow() |>
    testthat::expect_equal(
      d |> filter(day > as_date("2020-05-30")) |> nrow()
    )
  
  ## 测试时间格式比较
  gali_filter("dt", "time# >", "2020-05-30") |> task_run_dp(`@result` = d) |> nrow() |>
    testthat::expect_equal(
...
test_that("<gali_slice_max/ gali_slice_min>", {
  sample_config_init()
  ...
  task_create("cars/gali_read") |>
    task_item_gali_add(gali_read("车数据")) |>
    task_item_gali_add(gali_n_max("cyl", 3)) |>
    task_run() |>
    nrow() |>
  ...
  task_create("cars/gali_read") |>
    task_item_gali_add(gali_read("车数据")) |>
    task_item_gali_add(gali_n_min("disp", 3)) |>
    task_run() |>
    nrow() |>
    testthat::expect_equal(3)
...
test_that("<gali_select>", {
  ...
  task_create("cars/gali_read") |>
    task_item_gali_add(gali_read("车数据")) |>
    task_item_gali_add(gali_select("cyl")) |>
    task_item_gali_add(gali_collect()) |>
    task_run() |>
    ncol() |>
  ...
  task_create("cars/gali_read") |>
    task_item_gali_add(gali_read("车数据")) |>
    task_item_gali_add(gali_select(c("cyl", "disp"))) |>
    task_item_gali_add(gali_collect()) |>
    task_run() |>
    ncol() |>
  ...
  task_create("cars/gali_read") |>
    task_item_gali_add(gali_read("车数据")) |>
    task_item_gali_add(gali_select(c("cyl", "disp"), b_everything = T)) |>
    task_item_gali_add(gali_collect()) |>
    task_run() |>
    ncol() |>
  ...
  task_create("cars/gali_read") |>
    task_item_gali_add(gali_read("车数据")) |>
    task_item_gali_add(gali_select(c("cyl", "disp"), s_regex = "^@")) |>
    task_item_gali_add(gali_collect()) |>
    task_run() |>
    ncol() |>
...
test_that("<gali_arrange>", {
  ...
  (task_create("cars/gali_read") |>
    task_item_gali_add(gali_read("车数据")) |>
    task_item_gali_add(gali_arrange("disp")) |>
    task_item_gali_add(gali_collect()) |>
    task_run() |>
    head(1))$disp |>
  ...
  (task_create("cars/gali_read") |>
      task_item_gali_add(gali_read("车数据")) |>
      task_item_gali_add(gali_arrange("disp", b_desc = T)) |>
      task_item_gali_add(gali_collect()) |>
      task_run() |>
      head(1))$disp |>
...
test_that("<gali_rename>", {
  ...
  task_create("cars/gali_read") |>
    task_item_gali_add(gali_read("车数据")) |>
    task_item_gali_add(gali_rename("MY_DISP", "disp")) |>
    task_item_gali_add(gali_rename("中国队", "cyl")) |>
    task_item_gali_add(gali_select(c("中国队", "MY_DISP"))) |>
    task_item_gali_add(gali_collect()) |>
    task_run() |>
    ncol() |>
    testthat::expect_equal(2)
